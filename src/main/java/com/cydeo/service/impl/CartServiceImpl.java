...
public class CartServiceImpl implements CartService {
    ...
    public boolean deleteFromCart(UUID productId) {
        ...
        CartItem cartToDelete = CART.getCartItemList().stream()
                .filter(cartItem -> cartItem.getProduct().getId().equals(productId))
                .findAny().orElseThrow();

        //update cart total amount

        CART.setCartTotalAmount(CART.getCartTotalAmount().subtract(cartToDelete.getTotalAmount()));

        return CART.getCartItemList().remove(cartToDelete);
    }
}
